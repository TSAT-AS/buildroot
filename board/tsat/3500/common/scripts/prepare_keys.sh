...
if [ "$BUILD_TYPE" = "PROD" ]; then
  ...
  # decrypt keys
  gpg --batch --yes --status-file "$1/keys.log" --decrypt --armor --output "$WORK/efuse.nky" "$1/efuse.nky.enc"
  gpg --batch --yes --status-file "$1/keys.log" --decrypt --armor --output "$WORK/appfs.key" "$1/appfs.key.enc"
